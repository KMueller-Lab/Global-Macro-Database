...
foreach var of loc varnames {
	...
	* Export 
	qui export delimited using "$data_distr/`var'_`version'.csv", replace datafmt
...
bys ISO3: gen x = USDfx if year == $base_year
...
export delimited using "$data_distr/rGDP_USD_`version'.csv", replace datafmt
...
export delimited using "$data_distr/SovDebtCrisis_`version'.csv", replace datafmt
...
export delimited using "$data_distr/CurrencyCrisis_`version'.csv", replace datafmt
...
export delimited using "$data_distr/BankingCrisis_`version'.csv", replace datafmt


* Output the GMD as csv
use "$data_final/data_final", clear
export delimited using "$data_distr/GMD_`version'.csv", replace datafmt
export delimited using "$data_distr/GMD.csv", replace datafmt
