...
qui splice, priority(new old) generate(M1) varname(M1) base_year(2014)  method("chainlink") save("NO")
qui splice, priority(new old) generate(M2) varname(M2) base_year(2014)  method("chainlink") save("NO")
...
* Rename
ren(A I P) (year gen_govexp gen_govexp_GDP)
...
* Convert units
replace gen_govexp = gen_govexp * 1000 if year >= 1951
...
* Rename
ren(A B C) (year cgovrev cgovtax)
...
* Convert units
replace cgovrev = cgovrev * 1000 if year >= 1946
replace cgovtax = cgovtax * 1000 if year >= 1946
...
* Rename
ren(A D S) (year cgovdef cgovdef_GDP)
...
* Convert units 
replace nGDP = nGDP * 1000 if year >= 1953

* Derive government finance levels 
gen cgovdebt = (cgovdebt_GDP * nGDP) / 100
gen cgovrev_GDP = (cgovrev / nGDP) * 100
gen cgovtax_GDP = (cgovtax / nGDP) * 100
